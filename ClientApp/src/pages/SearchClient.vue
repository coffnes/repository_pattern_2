<template>
  <div>
    ...
    <div class="request_options">
      ...
      <p>Выберите DateFrom и DateTo</p>
    ...
    <weather-list :weathers="filteredSearch"/>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    filterOnlyZeroes() {
      return this.filterByDate.filter((value) => value.temperature === 0);
    },
    filteredSearch() {
      return this.filterByDate.filter(weather => {
        return weather.city.toLowerCase().includes(this.search.toLowerCase());
      });
    },
  },
};
